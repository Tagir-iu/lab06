...
## 9.Локальная сборка
Для проверки работоспособности проекта была проведена локальная сборка. Результат команды cmake --build _build подтверждает, что все компоненты (включая приложение solver) успешно скомпилированы.
![Успешная локальная сборка](https://github.com/Tagir-iu/lab06/blob/main/artifacts/Снимок%20экрана%C2%A0—%202026-06-10%20в%C2%A001.56.43.png)

# Выполнение задания Homework (Интеграция приложения Solver)
